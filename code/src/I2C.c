...
void I2C_GPIO_Init(void) {
	...
	// Set alternative function mode to I2C1
	...
	// Both GPIO pins should have a open-drain output type.
...
void I2C_Initialization(void){
	...
	// Set the system clock as the clock source for I2C1 in the peripherals independent clock
	...
	// Disable I2C while modifying control registers
	I2C1->CR1 &= ~(I2C_CR1_PE);

	I2C1->CR1 &= ~(I2C_CR1_ANFOFF); 		// enable analog noise filter
	I2C1->CR1 &= ~(I2C_CR1_DNF); 		// disble digital noise filter
	I2C1->CR1 |= I2C_CR1_ERRIE; 			// enable error interrupts
	I2C1->CR1 &= ~(I2C_CR1_NOSTRETCH); 	// enable clock stretching
	I2C1->CR2 &= ~(I2C_CR2_ADD10);		// set master to 7-bit addressing
	I2C1->CR2 |= I2C_CR2_AUTOEND;		// enable auto end mode
	I2C1->CR2 |= I2C_CR2_NACK;			// enable NACK generation

	I2C1->TIMINGR &= ~(I2C_TIMINGR_PRESC);
	I2C1->TIMINGR |= (7 << I2C_TIMINGR_PRESC_POS);   // PRESC = 7 -> t_PRESC = 0.1 us

	I2C1->TIMINGR &= ~(I2C_TIMINGR_SCLDEL);
  I2C1->TIMINGR |= (10 << I2C_TIMINGR_SCLDEL_POS);	 // SCLDEL = 10 -> t_SCLDEL > 1.0 us

	I2C1->TIMINGR &= ~(I2C_TIMINGR_SDADEL);
	I2C1->TIMINGR |= (12 << I2C_TIMINGR_SDADEL_POS); // SDADEL = 12 -> t_SDADEL > 1.25 us

	I2C1->TIMINGR &= ~(I2C_TIMINGR_SCLH);
	I2C1->TIMINGR |= (40 << I2C_TIMINGR_SCLH_POS);    // SCLH = 40 -> t_SCLH > 4.0 us

	I2C1->TIMINGR &= ~(I2C_TIMINGR_SCLL);
	I2C1->TIMINGR |= (47 << I2C_TIMINGR_SCLL_POS);    // SCLL = 47 -> t_SCLL > 4.7 us
	...
	// Enable I2C after finished modifying control registers
	I2C1->CR1 |= (I2C_CR1_PE);
}
